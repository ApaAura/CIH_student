...
namespace Problema6
{
    ...
    class Program
    {
        ...
        {
            //De la tastatură se citesc datele a trei elev: numele, prenumele și nota medie la o disciplină. Să se scire într-un fișier text doar elevii numele cărora se începe cu o consoană.
            Elev[] tab = new Elev[3];
            for (int i = 0; i < 3; i++)
            {
            ...
            using (StreamWriter wr = new StreamWriter("Fisier.txt"))
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (vocale.Contains(t[0]) == false) wr.WriteLine("{0} {1}        {2}", tab[i].nume, tab[i].prenume, tab[i].notaMed);
                }
            }
            Console.ReadKey();
        }
